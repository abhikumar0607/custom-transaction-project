                ...
                <div class="table-row">
                  <div class="table-sacroll">
                    <div class="table-inner">
                      <div class="box-open">
                        <ul>
                          <li class="select-li" id="groups-selected"></li>
                         ...
                         @php
                          ...
                          @endforeach
                            <tr class="nested_main_tr accordion-toggle" data-group_id="{{ $detail }}" data-row_count="<?php echo $count ?>" data-toggle="collapse" data-target="#demo<?php echo $count ?>" aria-expanded="false">
                                ...
                                <td>{{ \Carbon\Carbon::parse($groupData[0]['updated_at'])->format('d/m/Y') }}</td>
                                <!-- Display inbound and outbound quantities -->
                                <td><em>{{ $outbound_qty }}</em>/{{ $inbound_qty }}</td>
                                <!-- Display inbound and outbound weights -->
...
<script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (groupID) {
                // Activate the accordion for the specified group_id
                var accordionRow = $('tr[data-group_id="' + groupID + '"]');
                accordionRow.addClass('show');
                accordionRow.attr('aria-expanded', 'true');
            }
        });
    </script>
...
<script>
$(document).ready(function () {
    // Add change event listener to checkboxes inside the table
    $('#customDataTable').on('change', '.check-box', function() {
        // Check if any checkbox inside the table is checked
        if ($('#customDataTable .check-box:checked').length > 0) {
            // Add class 'your-class-name' to the .box-open div outside the table
            $('.box-open').addClass('menuitemshow');
        } else {
            // Remove class 'your-class-name' from the .box-open div outside the table
  ...
  $(document).ready(function () {
    ...
    $(document).on('click', '.accordion-toggle', function () {
        ...
        if (!isGrown) {
            ...
            currentAccordionBody.slideDown().addClass('in');;
        }
    });
...
$(document).ready(function() {
    ...
    // Bind change event to dynamically loaded checkboxes
    $('#customDataTable').on('change', '.check-box', handleCheckboxChange);

    // Call the function initially to handle existing checkboxes
