              ...
              <div class="search-upload-box">
                ...
                <div class="filter-btn">
                  <div class="btn-group dropdown-filter">
                    ...
                    <form class="from-filter" action="{{ route('customer.search.draft.transaction') }}" method="get">
                        <h4>DATE</h4>
                        <div class="date-left">
            ...
            <div class="col-md-6">
              ...
              <?php $counter = 0; 
                ...
                <div class="button-draft">
                  <button><img src="{{ asset('public/assets/img/logo/icon-svg1.svg') }}">Draft - <em><?php echo $counter; ?></em></button>
                ...
                <table class="table table-condensed" id="declaration_table" style="border-collapse:collapse;">
                  ...
                  @foreach($transaction as $data)
                     @foreach($data['transaction_history'] as $history)
                      ...
                      @endforeach
                      @php $count = 1; @endphp 
                    <tr class="accordion-toggle nested_main_tr" data-group_id="{{ $data['id'] }}" data-row_count="<?php echo $count ?>" data-toggle="collapse" data-target="#demo<?php echo $data['id']?>" data-status="{{ $data['status'] }}"  @if($data['status'] === 'draft') class="disabled-row" @endif >
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="oupload-poupbxx">
            ...
            <form id="uploadForm" action="{{ route('customer.submit.boe.pdf.file') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="refrencs">
  ...
  $(document).ready(function() {
    $('body').on('click', '.nested_main_tr', function() {
        ...
        $.ajax({
          ...
          url: base_url+'/draft-nested-records', 
          data: {
                  group_id: group_id,
                  row_count: row_count,
                },
       ...
       jQuery.ajax({  
         ...
         success: function(response) {
            ...
            if (response.includes("File Submitted Successfully.")) {
                ...
                // If the response does not contain the success message, handle accordingly
